...
- **Identifier provenance.** Sourcetypes are checked two ways. Every value
  written in query position, after `sourcetype=` or inside
  `sourcetype IN (...)`, must be catalogued in the Splunkbase catalogue or the
  CIM alignment reference, whatever its shape: `WinEventLog`, `fgt_traffic`
  and `zscalernss-web` are read by position, the same way the Sentinel check
  below reads tables. Separately, every colon-delimited token (`cisco:asa`)
...
- [scripts/record_golden_run.py](scripts/record_golden_run.py): writes [examples/golden-run.json](examples/golden-run.json), a content hash of every file the model run depends on, taken at the moment of the run; the validator compares it to the tree and prints a NOTICE naming what changed since; `--result` must record every fixture passing, so a partial or failing run cannot silence the notice
- [scripts/tests/test_record_golden_run.py](scripts/tests/test_record_golden_run.py): unit tests for the marker: line endings do not change the hash, the watched set is a rule rather than a list, and only a run of every fixture with every fixture passing is recorded, by the runner or by hand
